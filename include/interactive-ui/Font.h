...
struct Font
{
    ...
    consteval Font(uint8_t char_height, uint8_t char_width, uint8_t char_spacing, const uint8_t* char_bmp_data, size_t bmp_data_size)
        : char_height(char_height), char_width(char_width), char_spacing(char_spacing), char_bitmap(make_array_view(char_bmp_data, bmp_data_size))
    {
    }
    ...
    {
        return Font(char_height, char_width, char_spacing, data, char_width * (ascii_end - ascii_begin));
    }
