...
ClockComponent::ClockComponent(Screen* screen, RealTimeClock* clock, int32_t z_layer)
    : TextComponent(screen, 1.0, 0.1, clock->GetTimeString(), &clock_font, z_layer), clock(clock) // Aiming for top-right corner. 100% x-axis, 10% y-axis.
{
...
void ClockComponent::Draw()
{
    ...
    data.display->DrawText(origin_position - draw_dimensions, text, font, 0xFFFFFFFF);
}